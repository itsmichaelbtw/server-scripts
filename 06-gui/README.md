...
The `06-gui` directory contains the following scripts executed in sequence:
...
```bash
sudo /path/to/06-gui/00-netdata/run.sh
```
...
```bash
sudo /path/to/06-gui/01-filebrowser/run.sh
```
...
```bash
sudo /path/to/06-gui/02-homer/run.sh
```
...
```bash
sudo /path/to/06-gui/03-crontab-ui/run.sh
```
...
```bash
sudo /path/to/06-gui/04-gatus/run.sh
```
